
\usepackage{fontspec}
\setmainfont[BoldFont={SourceHanSansSC-Medium}]{SourceHanSerifSC-Light}
\setCJKmainfont[BoldFont={SourceHanSansSC-Medium}, ItalicFont={STFangsong}]{SourceHanSerifSC-Light}
\setsansfont[BoldFont={PingFang SC Medium}]{PingFang SC Regular}
\setCJKsansfont[BoldFont={PingFang SC Medium}, ItalicFont={PingFang SC Light}]{PingFang SC Regular}
\setCJKmonofont[BoldFont={Kaiti SC Bold}]{Kaiti SC Regular}

\setCJKfamilyfont{libian}{FZHJJW--GB1-0} \newcommand{\libian}{\CJKfamily{libian}}

\newfontfamily\AppleSymbolse{AppleSymbols}
\setCJKfamilyfont{AppleSymbols}{AppleSymbols}
\newcommand{\AppleSymbols}{\AppleSymbolse\CJKfamily{AppleSymbols}}

\newfontfamily\kaie[BoldFont={STKaitiSC-Black}]{STKaitiSC-Regular}
\setCJKfamilyfont{kaiti}[BoldFont={STKaitiSC-Black}]{STKaitiSC-Regular}
\newcommand{\kai}{\kaie\CJKfamily{kaiti}}

\newfontfamily\Copperplatee{Copperplate}
\setCJKfamilyfont{Copperplate}{Copperplate}
\newcommand{\Copperplate}{\Copperplatee\CJKfamily{Copperplate}}

\newfontfamily\xinge[BoldFont={STXingkaiSC-Bold}]{STXingkaiSC-Light}
\setCJKfamilyfont{xing}[BoldFont={STXingkaiSC-Bold}]{STXingkaiSC-Light}
\newcommand{\xing}{\xinge\CJKfamily{xing}}

\newfontfamily\gothice{SourceHanSans-Medium}
\setCJKfamilyfont{gothic}{SourceHanSans-Medium}
\newcommand{\gothic}{\gothice\CJKfamily{gothic}}

\newfontfamily\heie{SourceHanSansSC-Medium}
\setCJKfamilyfont{heiti}{SourceHanSansSC-Medium}
\newcommand{\hei}{\heie\CJKfamily{heiti}}

\newfontfamily\xihe{STXihei}
\setCJKfamilyfont{xihei}{STXihei}
\newcommand{\xihei}{\xihe\CJKfamily{xihei}}

\newfontfamily\zhongse[BoldFont={SourceHanSerifSC-Heavy}]{SourceHanSerifSC-Bold}
\setCJKfamilyfont{zhongs}[BoldFont={SourceHanSerifSC-Heavy}]{SourceHanSerifSC-Bold}
\newcommand{\zhongs}{\zhongse\CJKfamily{zhongs}}

\newfontfamily\toppanbe{Toppan Bunkyu Gothic Demibold}
\setCJKfamilyfont{toppanb}{Toppan Bunkyu Gothic Demibold}
\newcommand{\toppanb}{\toppanbe\CJKfamily{toppanb}}

\newfontfamily\kashoe[BoldFont={YuKyokasho Yoko Bold}]{YuKyokasho Yoko Medium}
\setCJKfamilyfont{kasho}[BoldFont={YuKyokasho Yoko Bold}]{YuKyokasho Yoko Medium}
\newcommand{\kasho}{\kashoe\CJKfamily{kasho}}

\newfontfamily\minchoe[BoldFont={SourceHanSerif-SemiBold}]{SourceHanSerif-Light}
\setCJKfamilyfont{mincho}[BoldFont={SourceHanSerif-SemiBold}]{SourceHanSerif-Light}
\newcommand{\mincho}{\minchoe\CJKfamily{mincho}}

\newfontfamily\minchobe[BoldFont={SourceHanSerif-Heavy}]{SourceHanSerif-Bold}
\setCJKfamilyfont{minchob}[BoldFont={SourceHanSerif-Heavy}]{SourceHanSerif-Bold}
\newcommand{\minchob}{\minchobe\CJKfamily{minchob}}

\newfontfamily\marue[BoldFont={DFMaruGothic-SB-WING-RKSJ-H}]{DFMaruGothic-Md-WING-RKSJ-H}
\setCJKfamilyfont{maru}[BoldFont={DFMaruGothic-SB-WING-RKSJ-H}]{DFMaruGothic-Md-WING-RKSJ-H}
\newcommand{\maru}{\marue\CJKfamily{maru}}

\newfontfamily\shingoe[BoldFont={ShinGoPro-Medium}]{ShinGoPro-Light}
\setCJKfamilyfont{shingo}[BoldFont={ShinGoPro-Medium}]{ShinGoPro-Light}
\newcommand{\shingo}{\shingoe\CJKfamily{shingo}}

\newfontfamily\outaie{OutaiKaiStd-Light}
\setCJKfamilyfont{outai}{OutaiKaiStd-Light}
\newcommand{\outai}{\outaie\CJKfamily{outai}}

\newfontfamily\tsukue[BoldFont={TsukuARdGothic-Bold}]{TsukuARdGothic-Regular}
\setCJKfamilyfont{tsuku}[BoldFont={TsukuARdGothic-Bold}]{TsukuARdGothic-Regular}
\newcommand{\tsuku}{\tsukue\CJKfamily{tsuku}}

\newcommand{\Chapter}[1]{\chapter*{#1\markboth{#1}{}}\addcontentsline{toc}{chapter}{#1}}
\renewcommand{\B}[1]{{\bfseries{#1}}}
